 ...
 * 让人当场改比回讨论里再聊三轮快得多）。
 */
export default function Confirm({ brief, consensus, generating, onChange, onConfirm, onBackToDiscuss }: Props) {
    ...
    <div className="space-y-5">
        ...
        {FIELDS.map(({ key, label, hint, rows }) => (
          <div key={key} className="rounded-2xl border border-neutral-200 bg-white p-3.5">
            ...
            <textarea
              ...
              placeholder={hint}
              disabled={generating}
              className="w-full resize-none rounded-lg border border-neutral-200 px-3 py-2 text-sm leading-relaxed outline-none focus:border-neutral-900 disabled:bg-neutral-50 disabled:text-neutral-400"
            />
          </div>
        ))}
      </div>
      ...
      <div className="space-y-2 pb-2">
        <button
          ...
          disabled={generating}
          className="w-full rounded-xl bg-neutral-900 py-3.5 font-medium text-white disabled:opacity-50"
        >
          {generating ? (
        ...
        <button
          ...
          disabled={generating}
          className="w-full rounded-xl py-2.5 text-sm text-neutral-500 disabled:opacity-40"
        >
          还有没聊到的，回去接着聊
        </button>
      </div>
