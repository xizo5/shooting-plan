...
- **「聊」和「定」分开**：归纳完约定后进入第 2 步的**独立确认页**——只有约定的五个字段可逐条改，不带聊天记录，改完点生成才走第 3 步。避免在长对话里翻找要改的那句话
...
见 [CONTEXT.md](CONTEXT.md)：前置条件（Brief）、步骤条、方案讨论、拍摄约定（Consensus）、确认页、流式回复、场景变体、策划（Shoot Plan）、场景（Scene）、画面（Shot）、参考图、参考片、插画库。
